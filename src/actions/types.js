// USER DATA RELATED ACTION TYPES
export const GET_ACCESS_TOKEN       = 'GET_ACCESS_TOKEN';
export const GET_ATHLETE            = 'GET_ATHLETE';
export const SET_USER_ERROR         = 'SET_USER_ERROR';
export const UPDATE_ATHLETE         = 'UPDATE_ATHLETE';
export const CHANGING_WEIGHT        = 'CHANGING_WEIGHT';
export const CHANGED_WEIGHT         = 'CHANGED_WEIGHT';



// UTILITIES ACTION TYPES
export const SET_MODAL_CONTENT      = 'SET_MODAL_CONTENT';
export const EMPTY_MODAL_CONTENT    = 'EMPTY_MODAL_CONTENT';
export const SET_MODAL_ERRORS       = 'SET_MODAL_ERRORS';
export const SET_LOADING_ELEMENT    = 'SET_LOADING_ELEMENT';
export const UNSET_LOADING_ELEMENT  = 'UNSET_LOADING_ELEMENT';
export const FLIP_NEW_ACTIVITY_FLAG = 'FLIP_NEW_ACTIVITY_FLAG';



// ACTIVITIES RELATED ACTION TYPES
export const CREATE_ACTIVITY        = 'CREATE_ACTIVITY';
export const GET_ACTIVITIES         = 'GET_ACTIVITIES';


